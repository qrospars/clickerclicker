...
public class NodeBasedEditor : EditorWindow
{
    ...
    {
        var paths = AssetDatabase.FindAssets("t:Weapon");

        var gameObjects = new List<GameObject>();

        var weaponsString = "Weapons:\n";

        foreach (var s in paths)
            weaponsString +=
                (AssetDatabase.LoadMainAssetAtPath(AssetDatabase.GUIDToAssetPath(s)) as Weapon).ToString() + "\n";
    ...
    private void OnGUI()
    {
        ...
        if (GUI.Button(new Rect(0, 0, 100, 30), "Save")) SaveNodes();
    ...
    private void ProcessContextMenu(Vector2 mousePosition)
    {
        ...
        genericMenu.AddItem(new GUIContent("Add Weapon"), false,
            () => OnClickAddNode(mousePosition, new DrawableWeapon()));
    ...
    private void OnClickAddNode(Vector2 mousePosition, DrawableInfo info)
    {
        ...
        nodes.Add(new Node(mousePosition, 350, 50, nodeStyle, selectedNodeStyle, inPointStyle, outPointStyle,
            OnClickInPoint, OnClickOutPoint, OnClickRemoveNode, info));
    }
    ...
    private void OnClickRemoveNode(Node node)
    {
        if (connections != null)
        {
            ...
            for (var i = 0; i < connectionsToRemove.Count; i++) connections.Remove(connectionsToRemove[i]);
    ...
    {
        var path = "Assets/Resources/Weapons";
        ...
        var nodesPassed = new List<Node>();

        foreach (var n in nodes)
            if (n.myInfo as DrawableWeapon != null)
            {
                var w = (n.myInfo as DrawableWeapon).GetWeapon();
                ...
                foreach (var c in connections)
                    if (c.inPoint.node == n)
                    {
                        ...
                        {
                            var w2 = LoadAsset<Weapon>(path + "/" + c.outPoint.node.myInfo.title + ".asset");
                    ...
                    else if (c.outPoint.node == n)
                    {
                        ...
                        {
                            var w2 = LoadAsset<Weapon>(path + "/" + c.inPoint.node.myInfo.title + ".asset");
                            w.upgradesTo.Add(w2);
